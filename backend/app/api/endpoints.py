...
async def export_pptx(request: ExportDocxRequest):
    """Build a simple slide deck from Markdown: one title slide, then one content slide
    per '## heading' with its lines as bullet points."""
    try:
        from pptx import Presentation
        from pptx.util import Inches, Pt
        import io as _io

        md = request.markdown_text or ""
        lines = md.split("\n")
        title = "Research Intelligence Report"
        slides = []  # list of (heading, [bullets])
        cur_head = None
        cur_bullets = []
        for ln in lines:
            s = ln.rstrip()
            if s.startswith("# ") and title == "Research Intelligence Report":
                title = s[2:].strip()
                continue
            if s.startswith("## "):
                if cur_head is not None:
                    slides.append((cur_head, cur_bullets))
                cur_head = s[3:].strip()
                cur_bullets = []
            elif s.strip():
        ...
        ts.shapes.title.text = title[:120]
        try:
            ts.placeholders[1].text = "Generated by Pinnovix Literature Intelligence"
        except Exception:
            pass
        # Content slides
        for head, bullets in slides:
            slide = prs.slides.add_slide(prs.slide_layouts[1])
            slide.shapes.title.text = head[:120]
            body = slide.placeholders[1].text_frame
            body.clear()
            shown = bullets[:12] if bullets else [""]
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": "attachment; filename=Pinnovix_Report.pptx"},
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
