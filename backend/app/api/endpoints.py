    ...
    try:
        ...
        async def event_generator():
            try:
                ...
                # Run the agent graph to completion. The agent nodes use model.invoke()
                # (not streaming) and some deployed langchain versions do not emit
                # on_chat_model_stream events, so we ainvoke and surface the final result.
                result = await app_graph.ainvoke(initial_state)
                final_messages = result.get("messages", []) if isinstance(result, dict) else []

                # Surface any structured paper results (literature review) from tool calls
                for m in final_messages:
                    if m.__class__.__name__ == "ToolMessage":
                        tc = getattr(m, "content", "")
                        if isinstance(tc, str) and tc.strip().startswith("[") and tc.strip().endswith("]"):
                            try:
                                papers = json.loads(tc)
                                yield f"data: {json.dumps({'type': 'papers', 'content': papers})}\n\n"
                            except Exception:
                                pass

                # Surface the final assistant answer
                final_text = ""
                if final_messages:
                    final_text = _extract_text(getattr(final_messages[-1], "content", ""))
                if final_text:
