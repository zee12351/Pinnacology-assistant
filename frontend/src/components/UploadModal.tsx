...
export function UploadModal({ showUploadModal, setShowUploadModal, handleFileUpload, uploadingDoc }: any) {
  ...
  const handleZoteroSync = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/library/zotero`, {
        method: 'POST',
  ...
  const handleMendeleySync = async () => {
    setIsFetchingLibrary(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/library/mendeley`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleFetchId = async () => {
    ...
    setIsFetchingLibrary(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/library/fetch-id`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
